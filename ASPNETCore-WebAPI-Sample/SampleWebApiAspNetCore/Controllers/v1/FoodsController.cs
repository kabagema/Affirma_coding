...
namespace SampleWebApiAspNetCore.v1.Controllers
{
  ...
  /*
        [HttpGet]
        [Route("SearchFoodbyCalories/{min:int}/ {max: int}", Name = nameof(SearchFoodbyCalories))]
        // search and return that is between a min and max calorie value
        public ActionResult SearchFoodbyCalories(int min, int max, [FromQuery] QueryParameters queryParameters)
        {
            // QueryParameters queryParameters = null;
            List<FoodItem> allFoodItem = _foodRepository.GetAll(queryParameters).ToList();
            List<FoodItem> toreturn = new List<FoodItem>();

            foreach (FoodItem x in allFoodItem)
            {
                if (Enumerable.Range(min, max).Contains(x.Calories))
                {
                    toreturn.Add(x);
                }
            }

            var toreturnCount = toreturn.Count();

            var paginationMetadata = new
            {
                totalCount = toreturnCount,
                pageSize = queryParameters.PageCount,
                currentPage = queryParameters.Page,
                totalPages = queryParameters.GetTotalPages(toreturnCount)
            };
             Response.Headers.Add("X-Pagination",
                Newtonsoft.Json.JsonConvert.SerializeObject(paginationMetadata));

            var links = CreateLinksForCollection(queryParameters, toreturnCount);

            var toReturn = toreturn.Select(x => ExpandSingleFoodItem(x));
...
*/
        ...
        // public change the food method to add a weight functionality
